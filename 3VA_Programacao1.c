...
void telaRelatorioEstatistico() {
    ...
    int locMaisNovo = 120; // Valor inicial alto para ajudar a encontrar o menor (difícilmente alguém com mais de 120 anos irá alugar);
...
float calcularValorAPagar(int qtdHospedes, int dias, int auxLimpeza) {
    ...
    if (qtdHospedes <= 5) {
        ...
        // Valor inválido de quantidade de hóspedes
        return 0.0;
    }
...
void lerDadosClientes(){
    ...
    while (fscanf(arquivo, "%25[^|]|%d|%d|%d|%d|%f|%f\n", Cliente.nome, &Cliente.idade, &Cliente.qtd, &Cliente.dias, &Cliente.auxLimpeza, &Cliente.valorTotal, &Cliente.adiantamento) == 7) {
        // Da linha 310 à 314 é uma "gambiarra" para fazer exibir os valores em float com o espaçamento adequado;
